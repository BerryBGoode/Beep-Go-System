    ...
    private void TbParqueosWhiteMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TbParqueosWhiteMouseClicked
        ...
        try {
            if (row < TbParqueosWhite.getRowCount() && row >= 0 && col < TbParqueosWhite.getColumnCount() && col >= 0) {
                ...
                //si detecta que en la posición q tiene "obj" es un jButton
                if (obj instanceof JButton) {
                    //detecte el evento click
                    ((JButton) obj).doClick();
                    JButton btn = (JButton) obj;
                    //diferenciar si es el de modificar o eliminar
    ...
    void getdataPark() {
        ...
        if (tbpark.getRowCount() > 0) {//si hay columnas antes de cargar 
            ...
            try {
                ...
                while (rs.next()) {
                    Object[] data = {rs.getInt("idDetalleAcceso"), rs.getDate("fecha"), rs.getInt("idAcceso"), rs.getTime("hora"), rs.getInt("idParqueo"), rs.getString("numero_carnet"), rs.getInt("idVehiculo"), rs.getString("nombre_p"), rs.getString("apellido_p"), rs.getString("placa"), rs.getString("color"), rs.getInt("numero_parqueo"), /*agregar en cada fila un btn*/ btnmodificar, btndelete};
                    tbpark.addRow(data);
